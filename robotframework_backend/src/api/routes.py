...
# PUBLIC_INTERFACE
@router.post("/run", response_model=RunResponse, summary="Start Robot run", description="Start a Robot Framework run with specified options.")
...
# PUBLIC_INTERFACE
@router.post("/stop", response_model=StopResponse, summary="Stop run", description="Stop the currently running Robot process if any.")
...
# PUBLIC_INTERFACE
@router.get("/stats", response_model=StatsResponse, summary="Run stats", description="Get aggregated statistics for a run.")
def stats(run_id: int = Query(..., description="Run id"), db: Session = Depends(get_db)) -> StatsResponse:
...
# PUBLIC_INTERFACE
@router.get("/case_status", response_model=CaseStatusResponse, summary="Case status counts", description="Return counts for passed/failed/recorded cases.")
def case_status(run_id: int = Query(..., description="Run id"), db: Session = Depends(get_db)) -> CaseStatusResponse:
...
# PUBLIC_INTERFACE
@router.get("/progress", response_model=ProgressResponse, summary="Run progress", description="Return computed progress for a run from expected total.")
def progress(run_id: int = Query(..., description="Run id"), db: Session = Depends(get_db)) -> ProgressResponse:
...
# PUBLIC_INTERFACE
@router.get("/current_case_info", response_model=CurrentCaseInfo, summary="Current case info", description="Get information about the currently executing test case.")
def current_case_info(run_id: int = Query(..., description="Run id")) -> CurrentCaseInfo:
...
# PUBLIC_INTERFACE
@router.get("/logs", summary="Stream logs (SSE)", description="Stream the current run logs via Server-Sent Events if enabled.")
async def logs():
...
# PUBLIC_INTERFACE
@router.get("/config", response_model=ConfigPayload, summary="Read config", description="Read YAML/JSON config content from CONFIG_DIR.")
def get_config(path: str = Query(..., description="Relative config path from CONFIG_DIR")) -> ConfigPayload:
...
# PUBLIC_INTERFACE
@router.put("/config", response_model=ConfigPayload, summary="Write config", description="Write YAML/JSON config content to CONFIG_DIR.")
...
# PUBLIC_INTERFACE
@router.get("/list_config_folders", response_model=ConfigListResponse, summary="List config folders", description="List folders under CONFIG_DIR.")
...
# PUBLIC_INTERFACE
@router.post("/ui_lock", summary="Set UI lock", description="Set or clear UI lock to prevent concurrent actions.")
...
# PUBLIC_INTERFACE
@router.post("/sync_state", response_model=GetStateResponse, summary="Sync state", description="Merge provided state into stored UI/app state.")
...
# PUBLIC_INTERFACE
@router.get("/get_state", response_model=GetStateResponse, summary="Get state", description="Return stored UI/app state.")
...
# PUBLIC_INTERFACE
@router.get("/expected_total", response_model=ExpectedTotalResponse, summary="Get expected total", description="Get expected total test cases for current run.")
async def get_expected_total(run_id: int = Query(..., description="Run id")) -> ExpectedTotalResponse:
...
# PUBLIC_INTERFACE
@router.post("/expected_total", response_model=ExpectedTotalResponse, summary="Set expected total", description="Set expected total test cases for current run.")
async def set_expected_total(run_id: int = Query(..., description="Run id"), total: int = Query(..., description="Expected total")) -> ExpectedTotalResponse:
